...
if len(args) < 2:
    abort(
        "Must have at least two arguments - the domain name and the memory for the Java heap. The memory can either be a number with a % on the end to give a percentage of available memory or end with an m or g to express it in mega- or giga-bytes"
    )
domain = args[0]
...
type = args[1][-1]
if type not in "%mg":
    abort("Memory must be specified as %, m or g")
...
try:
    ...
    executeGood(
        "asadmin -W " + tfile + " --user admin create-domain --savelogin " + domain
    )
finally:
    os.remove(tfile)
...
dq = '\\"'
executeGood(
    "asadmin create-jvm-options '-XX\:OnOutOfMemoryError=" + dq + cmd + dq + "'"
)
